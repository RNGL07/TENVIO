...
Businesses directory (`/admin/businesses`) and detail page (`/admin/businesses/[id]`) DONE — name, owner email, plan/price, customer count, subscription status, derived access, trial/period dates, Stripe customer id. **Not done**: actually changing the active Plan/price from the Admin Hub (still requires a direct DB edit via Prisma Studio or a Stripe Dashboard change) — flagged as real remaining work, not forgotten.

## Phase H — Admin Account Controls & Audit Logs

Restrict/reactivate and comp/uncomp DONE (`src/actions/admin-actions.ts`), each writes an `AdminAuditLog` row, and `/admin/audit` now renders that log (paginated, newest first, links back to each business). **Deliberately not built**: admin-initiated cancellation and account "terminate" — both need Aaron to define exact semantics (immediate vs. at-period-end for admin-cancel; whether terminate deletes data or just permanently locks it) before building, not a guess call.
